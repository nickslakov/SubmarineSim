...
def rnn(data):
    ...
    l_hid1 = lasagne.layers.GRULayer(l_in, num_units=20)
    l_reshape1 = lasagne.layers.ReshapeLayer(l_hid1,(-1,[2]))
    l_out = lasagne.layers.DenseLayer(l_reshape1, num_units=NUM_OUTPUTS)
    l_out = lasagne.layers.ReshapeLayer(l_out, (-1, 49, 2))
    ...
    num_updates = []
    train_cost, val_cost = [],[]
    x_current = np.zeros((BATCH_SIZE,SEQUENCE_LENGTH,NUM_INPUTS))
    y_current = np.zeros((BATCH_SIZE,SEQUENCE_LENGTH,NUM_OUTPUTS))

    for e in range(NUM_BATCHES):
        x_current = data[e,:,:-1]
        y_current = data[e,:,1:,:2]

        np.roll(y_current, SEQUENCE_LENGTH - 1)
